...
namespace TwinCAT.ProductivityTools
{
	public class Function
	{
		...
		public static async Task<IEnumerable<Function>> ListFunctionsAsync(
			AmsNetId target,
			...
			using (AdsClient client = new AdsClient())
			{
				...
				foreach (var key in RegistryDefinitions.Keys)
				{
					try
					{
						...
						var regKey =
							@"SOFTWARE\WOW6432Node\Beckhoff\TwinCAT3 Functions\"
							+ name
							+ @"\Common";

						var version = await AdsRegistry.QueryValueAsync(target, regKey, "Version");
		...
		private static Dictionary<FunctionType, string> RegistryDefinitions = new Dictionary<
			FunctionType,
			string
		>
		{
			...
			{
				FunctionType.TF8010_BuildingAutomationBasic,
				"Beckhoff TF8010 Building Automation Basic"
			},
